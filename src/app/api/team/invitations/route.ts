...
export async function POST(request: NextRequest) { try { return NextResponse.json(await inviteMember(await request.json(), { ...(await requireRequestContext(request)), requestOrigin: request.nextUrl.origin }), { status: 201 }); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Erreur serveur" }, { status: 400 }); } }
