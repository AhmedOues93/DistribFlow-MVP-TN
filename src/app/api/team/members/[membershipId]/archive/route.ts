...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ membershipId: string }> }) { try { return NextResponse.json(await changeMemberStatus((await params).membershipId, MembershipStatus.ARCHIVED, await requireRequestContext(request))); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Erreur serveur" }, { status: 400 }); } }
