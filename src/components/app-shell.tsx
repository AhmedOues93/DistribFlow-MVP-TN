...
import { Boxes, ChevronDown, ClipboardList, LayoutDashboard, LogOut, Menu, Package, Search, Settings, UserRound, Users, Warehouse, X } from "lucide-react";
...
const catalogueRoles = ["PLATFORM_ADMIN", "OWNER", "WAREHOUSE_MANAGER", "VIEWER"];
const stockRoles = ["PLATFORM_ADMIN", "OWNER", "WAREHOUSE_MANAGER", "VIEWER"];

const navigation: NavigationGroup[] = [
  { label: "Pilotage", items: [{ href: "/dashboard", label: "Tableau de bord", icon: LayoutDashboard }] },
  { label: "Ventes", items: [
    { href: "/clients", label: "Clients", icon: Users, roles: ["PLATFORM_ADMIN", "OWNER", "SALES_AGENT", "ACCOUNTANT", "VIEWER"] },
...
export function AppShell({ children, session }: { children: React.ReactNode; session: { userName: string; tenantName: string; role: string } }) {
  ...
  return <RoleContext.Provider value={session.role}><div className="shell app-shell">
    ...
    <aside className={open ? "sidebar open" : "sidebar"}>
      ...
      <div className="sidebar-footer"><span className="status-dot" />Données sécurisées par entreprise</div>
    </aside>
