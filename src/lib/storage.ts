...
export function safeStorageKey(key: string) { if (!/^[a-zA-Z0-9/_-]+\.(png|jpg|jpeg|webp|pdf|txt|csv)$/.test(key) || key.includes("..")) throw new Error("Clé de stockage invalide"); return key; }

class LocalStorage implements ObjectStorage {
  private root = process.env.LOCAL_STORAGE_DIR ?? join(process.cwd(), ".local-storage");
  private path(key: string) { const safe = safeStorageKey(key); const target = normalize(join(this.root, safe)); if (!target.startsWith(normalize(this.root) + "/")) throw new Error("Clé de stockage invalide"); return target; }
...
function contentTypeFor(key: string) { return key.endsWith(".png") ? "image/png" : key.endsWith(".webp") ? "image/webp" : key.endsWith(".pdf") ? "application/pdf" : key.endsWith(".txt") ? "text/plain" : key.endsWith(".csv") ? "text/csv" : "image/jpeg"; }
...
class S3Storage implements ObjectStorage {
  ...
  private async request(method: string, key: string, body?: Buffer, contentType?: string) {
    ...
    const endpoint = new URL(this.endpoint); const path = `${endpoint.pathname.replace(/\/$/, "")}/${encodedPath(this.bucket)}/${encodedPath(safeStorageKey(key))}`; const url = new URL(path, endpoint);
